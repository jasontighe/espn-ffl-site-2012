package com.espn.ffl.webcam.http {
	import com.upload.UploadHelper;
	...
	public class FFLUploadHelper 
	{
		...
		public static function getPostData( fileName:String, byteArray:ByteArray, parameters:Object = null):ByteArray {
			...
			postData = BOUNDARY(postData);
			postData = LINEBREAK(postData);
			bytes = 'Content-Disposition: form-data; name="userfile"; filename="';
			for ( i = 0; i < bytes.length; i++ ) {
				postData.writeByte( bytes.charCodeAt(i) );
			}
		...
		private static function BOUNDARY(p:ByteArray):ByteArray {
			var l:int = UploadHelper.getBoundary().length;
