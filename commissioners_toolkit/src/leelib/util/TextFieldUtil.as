package leelib.util {
	...
	{ 
		...
		public static function makeTextWithCopyDto($dto:CopyDTO, $forcedWidth:Number=NaN, $forcedHeight:Number=NaN, $stylesheet:StyleSheet=null):TextField
		{
			var tf:TextField = makeText($dto.copy, $dto.style, $forcedWidth, $forcedHeight, $stylesheet);
			
			// note how x/y might be set either within style (via "addValue") or from dto x/yPos
			if (! isNaN($dto.xPos)) tf.x = $dto.xPos;
			if (! isNaN($dto.yPos)) tf.y = $dto.yPos;
			
			return tf;
		}
		
		public static function makeHtmlTextWithCopyDto($dto:CopyDTO, $forcedWidth:Number=NaN, $forcedHeight:Number=NaN, $stylesheet:StyleSheet=null):TextField
		{
			var tf:TextField = makeHtmlText($dto.copy, $dto.style, $forcedWidth, $forcedHeight, $stylesheet);
		...
		public static function makeTextWithEncodingDto($dto:EncodingDTO, $forcedWidth:Number=NaN, $forcedHeight:Number=NaN, $stylesheet:StyleSheet=null):TextField
		{
			var tf:TextField = makeText($dto.copy, $dto.style, $forcedWidth, $forcedHeight, $stylesheet);
		...
		public static function makeHtmlTextWithEncodingDto($dto:EncodingDTO, $forcedWidth:Number=NaN, $forcedHeight:Number=NaN, $stylesheet:StyleSheet=null):TextField
		{
			var tf:TextField = makeHtmlText($dto.copy, $dto.style, $forcedWidth, $forcedHeight, $stylesheet);
		...
		public static function makeTextQuick($text:String, $size:uint=12, $color:uint=0x0):TextField
		{
			...
			tf.text = $text;
			tf.filters = [ new ColorMatrixFilter() ]; // har
			tf.autoSize = TextFieldAutoSize.LEFT;
			tf.mouseEnabled= false;
		...
		public static function makeTextFormatFromStyleName($styleName:String, $stylesheet:StyleSheet=null):TextFormat
		{
			if (! $stylesheet) $stylesheet = _defaultStyleSheet; 
			var style:Object = $stylesheet.getStyle($styleName);
			return $stylesheet.transform(style);  
		...
		public static function ellipsize($tf:TextField, $width:Number):void
		{
			...
			while ($tf.textWidth > $width && $tf.text.length > 2)
		}
