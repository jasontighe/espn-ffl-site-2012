package leelib.vid
{
	...
	// TODO: Redo or just remove UI component tie-ins, which currently are bunk
	//
	public class MinVid extends Component
	{
		...
		private var _nativeVideoWidth:Number;
		private var _nativeVideoHeight:Number;
		private var _ignoreNativeVideoAspectRatio:Boolean;
		...
		private var _wasPlayingBeforeDrag:Boolean;
		...
		public function go($url:String, $ignoreNativeVideoAspectRatio:Boolean=false, $autoStart:Boolean=true, $autoRepeat:Boolean=false):void
		{
			if (! _sizeWidth || ! _sizeHeight) {
				throw new Error("Set sizeWidth and sizeHeight first plz");
			}
			
			_url = $url;
			_ignoreNativeVideoAspectRatio = $ignoreNativeVideoAspectRatio;
			...
			if (_progressBar) _progressBar.playElement.visible = false; // not sure about this exactly
		...
		public function get nativeVideoWidth():Number
		{
			return _nativeVideoWidth;
		}
		public function get nativeVideoHeight():Number
		{
			return _nativeVideoHeight;
		}
		...
		public function play():void
		{
			...
			_video.visible = true;
			if (_progressBar) _progressBar.playElement.visible = true;
		...
		public function set metaData(info:Object):void
		{
			...
			_nativeVideoWidth = parseFloat(info.width); 
			_nativeVideoHeight = parseFloat(info.height); 
			
			size();
			
			setTimeout(function():void{_video.visible = true;}, 150); // !!
		}
		...
		{
			if (_ignoreNativeVideoAspectRatio || ! _nativeVideoWidth || ! _nativeVideoHeight)
			{
				_video.width = _sizeWidth;
				_video.height = _sizeHeight;
			}
			else
			{
				// (rem, 'GrUtil.fitInRect' won't work here because of stupid Video sizing behavior) 
				var r:Rectangle = GrUtil.getFittedRect(_nativeVideoWidth, _nativeVideoHeight, _sizeWidth, _sizeHeight);
				GrUtil.applyRectangleToDisplayObject(_video, r);
			}
		}
		...
		{
			_isDragging = true;
			_wasPlayingBeforeDrag = (_state != STATE_PAUSED);
			pause();
		}
		private function onVidProgressBarDragEnd(e:*):void
		{
			_isDragging = false;
			if (_wasPlayingBeforeDrag) resume();
		}
